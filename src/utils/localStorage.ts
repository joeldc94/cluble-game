...
export const setNewAnswer = (gameId: string, answer: string) => {
    ...
    if (history) {
        const gameIndex = history.findIndex(
            (game: GameHistoryLocalStorage) => game.gameId === gameId
        );
        if (gameIndex !== -1) {
...
/** Insere novo jogo no registro. Se já existe um registro, retorna a quantidade de respostas dadas */
export const setLocalStorageNewGame = (gameId: string) => {
    ...
    if (history) {
        ...
        return history[gameIndex].answers.length;
    }
    return 0;
};
